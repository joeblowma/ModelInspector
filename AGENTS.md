...
This is a small Python desktop/CLI utility for inspecting `.safetensors` model files.
...
- `src/` stores bundled application assets, such as icons used by the GUI and PyInstaller build.
- `requirements.txt` lists runtime/build dependencies.
- `compile.bat`, `SafetensorsModelInspector.spec`, `build/`, and `dist/` support PyInstaller packaging. Treat `build/` and `dist/` as generated output.
...
- `py gui.py` launches the desktop UI.
- `py inspect_model.py --help` checks CLI argument wiring.
- `py inspect_model.py path\to\model.safetensors` inspects one file from the CLI.
- `py inspect_model.py path\to\folder --recursive --json` runs a recursive CLI smoke test with JSON output.
